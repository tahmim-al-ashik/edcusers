...
class BroadbandCheckoutURLController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        if($client){
            $settingsInfo = CorporateClientsSettings::where('client_uid', '=', $user_id)->first(['bkash_username','bkash_password','bkash_app_key','bkash_app_secret_key']);
        }elseif($agent){
        ...
        if($sub_agent){
            $editor_id_from_sub_agent = CorporateSubAgent::where('uid',$user_id)->value('client_id');
            $branchInfo = CorporateClient::where('uid', '=', $editor_id_from_sub_agent)->first(['mikrotik_ip','mikrotik_username','mikrotik_password']);
        }elseif($agent){
            $editor_id_from_agent = CorporateAgent::where('uid',$user_id)->value('client_id');
            $branchInfo = CorporateClient::where('uid', '=', $editor_id_from_agent)->first(['mikrotik_ip','mikrotik_username','mikrotik_password']);
        }elseif($client){
            $branchInfo = CorporateClient::where('uid', '=', $user_id)->first(['mikrotik_ip','mikrotik_username','mikrotik_password']);
        }
        ...
        // Api Variables
        $ipAddr = $branchInfo->mikrotik_ip;
        $mkUser = $branchInfo->mikrotik_username;
        $mkPass = $branchInfo->mikrotik_password;
        $API = new RouterOsApi();
            ...
            if((!isset($arr['trxID']) || empty($arr['trxID'])) && array_key_exists("statusCode",$arr) && $arr['statusCode'] != '0000'){
                ...
                if($oldUserCheck){
                    InternetUsers::where('uid', $internet_user_id)->update([
                        ...
                        'package_expire_date' => Carbon::now()->addDays(30)
                    ]);
                }else{
                    InternetUsers::where('uid', $internet_user_id)->update([
                        'connection_status' => 'active',
                ...
                // Fetch branch information
                $branchInfo = CorporateClient::where('uid', '=', $zone_id)->first(['mikrotik_ip','mikrotik_username','mikrotik_password']);
