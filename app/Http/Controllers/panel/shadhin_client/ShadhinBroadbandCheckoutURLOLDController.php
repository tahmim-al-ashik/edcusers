...
class ShadhinBroadbandCheckoutURLController extends Controller
{
    ...
    public function authHeaders(){
        Log::info("check-1:".$settingsInfo);
        $bkash_app_key = $settingsInfo['bkash_app_key'];
        Log::info("check-2:".$bkash_app_key);
        $status = 200;
        return array(
    ...
    public function create(Request $request)
    {
        ...
        $bkash_username = '01971399998';
        $settingsInfo = CorporateClientsSettings::where('bkash_username', '=', $bkash_username)->first(['bkash_username','bkash_password','bkash_app_key','bkash_app_secret_key']);
    ...
    public function callback(Request $request)
    {
        ...
        if(isset($allRequest['status']) && $allRequest['status'] == 'success'){
            ...
            if(array_key_exists("statusCode",$arr) && $arr['statusCode'] != '0000'){
                ...
                $mkUser = $branchInfo->implode('zone_username', ', ');
                $mkPass = $branchInfo->implode('zone_password', ', ');
                $zoneName = $branchInfo->implode('zone_name', ', ');
