...
{
    // Customer list ---
    public function transCustomerList(Request $request) : JsonResponse
    {
    ...
    public function transCustomerDetails($id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();

        // Fetch the transCustomer record along with related data
        $transCustomer = TransCustomer::leftJoin('trans_pops', 'trans_customers.pop_id', '=', 'trans_pops.id')
    ...
    // Create Customer ---
    public function createTransCustomer(Request $request) : JsonResponse
    {
            $returned_data = ResponseWrapper::Start();
    ...
    // Edit Customer ---
    public function editTransCustomer(Request $request, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $validated = $request->validate([
            'customer_name' => 'required',
    ...
    // Delete Customer ---
    public function deleteTransCustomer($id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $latLongDeleted = TransLatLong::where('trans_id', $id)->where('module_type','customer')->delete();
        $workerDeleted = TransWorkerInfo::where('trans_id', $id)->where('module_type','customer')->delete();

        // Fetch all images related to the pop
        $images = TransImage::where('trans_id', $id)->where('module_type','customer')->get();
        ...
        $imageDeleted = TransImage::where('trans_id', $id)->where('module_type','customer')->delete();
        $customerDeleted = TransCustomer::where('id', $id)->delete();
    ...
    // Summary Customer ---
    public function summaryTransCustomer() : JsonResponse {
        $returned_data = ResponseWrapper::Start();

        // summary
        $summary = TransCustomer::selectRaw(
    ...
    public function getTransCustomerLatLong(Request $request) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
