...
use App\Imports\TransReservedLoopImport;
...
class TransReservedLoopController extends Controller
{
    // Reserved Loop list ---
    public function transReservedLoopList(Request $request, $auth_id) : JsonResponse
    {
        ...
        $query = $query->where('loop_type','reserved_loop')
    ...
    // Reserved Loop Single Details ---
    public function transReservedLoopDetails($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        // Fetch the transReservedLoop record along with related data
        $transReservedLoop = TransLoop::query();
        if($base_role !== 'admin'){
            $transReservedLoop = $transReservedLoop->where('trans_loops.zone_id', $zone_id);
        }
        $transReservedLoop = $transReservedLoop->where('trans_loops.id', $id)
                            ->leftJoin('trans_pops', 'trans_loops.pop_id', '=', 'trans_pops.id')
                            ->leftJoin('trans_tj_boxes', 'trans_loops.tj_box_id', '=', 'trans_tj_boxes.id')
                            ->with([
                                'reservedCableDetails',
                                'reservedWorkerInfos',
                                'reservedImages',
                                'reservedLatLong'
                            ])
        ...
        if ($transReservedLoop) {
            $details = [
                'id' => $transReservedLoop->id,
                'pop_id' => $transReservedLoop->pop_id,
                'tj_box_id' => $transReservedLoop->tj_box_id,
                'olt_port' => $transReservedLoop->olt_port,
                'pop_code' => TransPop::where('id',$transReservedLoop->pop_id)->value('pop_code'),
                'tj_box_code' => TransTjBox::where('id',$transReservedLoop->tj_box_id)->value('tj_box_code'),
                'loop_code' => $transReservedLoop->loop_code,
                'loop_type' => $transReservedLoop->loop_type,
                'latitude' => $transReservedLoop->latitude,
                'longitude' => $transReservedLoop->longitude,
                'address_direction' => $transReservedLoop->address_direction,
                'added_by_uid' => $transReservedLoop->added_by_uid,
                'updated_by_uid' => $transReservedLoop->updated_by_uid,
                'comments' => $transReservedLoop->comments,
                'status' => $transReservedLoop->status,
                'created_at' => $transReservedLoop->created_at,
                'updated_at' => $transReservedLoop->updated_at,

                'cable_details' => $transReservedLoop->reservedCableDetails->map(function ($cableDetail) {
                    if($cableDetail->cable_type === 'reserved_loop_cable'){
                        return [
                            'id' => $cableDetail->id,
                ...
                'images' => $transReservedLoop->reservedImages->map(function ($image) {
                    return [
                        'id' => $image->id,
                ...
                'worker_info' => $transReservedLoop->reservedWorkerInfos->map(function ($workerInfo) {
                    return [
                        'id' => $workerInfo->id,
    ...
    // Create Reserved Loop ---
    public function createTransReservedLoop(Request $request, $auth_id) : JsonResponse
    {
            $returned_data = ResponseWrapper::Start();
            ...
            $address->module_type = 'reserved_loop';
            ...
            $cable->module_type = 'reserved_loop';
            $cable->cable_type = 'reserved_loop_cable';
            ...
            $workerInfo->module_type = 'reserved_loop';
            ...
            $returned_data['status'] = 'success';
            $returned_data['message'] = 'Reserved loop added successfully!';
            $returned_data['results'] = $loopId;
            return ResponseWrapper::End($returned_data);
    }

    // Edit Reserved Loop ---
    public function editTransReservedLoop(Request $request, $auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $validated = $request->validate([
            'loop_id' => 'required',
        ...
        $address = TransLatLong::where('trans_id', $id)->where('module_type','reserved_loop')->first();
        if($address){
            $address->update([
                'zone_id' => $zone_id,
                'trans_id' => $id,
                'module_type' => 'reserved_loop',
                'latitude' => $request->get('latitude'),
                'longitude' => $request->get('longitude'),
                'status' => 'Active'
            ]);
        ...
        $cableInfo = TransCableDetail::where('trans_id',$id)->where('module_type','reserved_loop')->where('cable_type','reserved_loop_cable')->first();
        $cableInfo->update([
            'zone_id' => $zone_id,
            'trans_id' => $id,
            'module_type' => 'reserved_loop',
            'cable_type' => 'reserved_loop_cable',
        ...
        $workerInfo = TransWorkerInfo::where('trans_id', $id)->where('module_type','reserved_loop')->first();
        if($workerInfo){
            $workerInfo->update([
                'zone_id' => $zone_id,
                'trans_id' => $id,
        ...
        $returned_data['status'] = 'success';
        $returned_data['message'] = 'Reserved loop data updated successfully!';
        return ResponseWrapper::End($returned_data);
    }

    // Delete Reserved Loop ---
    public function deleteTransReservedLoop($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $latLongDeleted = $latLongDeleted->where('trans_id', $id)->where('module_type','reserved_loop')->delete();

        $cableDeleted = TransCableDetail::query();
        if($base_role !== 'admin'){
            $cableDeleted = $cableDeleted->where('zone_id', $zone_id);
        }
        $cableDeleted = $cableDeleted->where('trans_id', $id)->where('module_type','reserved_loop')->delete();

        $workerDeleted = TransWorkerInfo::query();
        if($base_role !== 'admin'){
            $workerDeleted = $workerDeleted->where('zone_id', $zone_id);
        }
        $workerDeleted = $workerDeleted->where('trans_id', $id)->where('module_type','reserved_loop')->delete();
        ...
        $images = $images->where('trans_id', $id)->where('module_type','reserved_loop')->get();
        ...
        $imageDeleted = $imageDeleted->where('trans_id', $id)->where('module_type','reserved_loop')->delete();

        $loopDeleted = TransLoop::query();
        if($base_role !== 'admin'){
            $loopDeleted = $loopDeleted->where('zone_id', $zone_id);
        }
        $loopDeleted = $loopDeleted->where('id', $id)->where('loop_type','reserved_loop')->delete();
        if ($latLongDeleted && $cableDeleted && $workerDeleted && $loopDeleted) {
            $returned_data['status']  = 'success';
            $returned_data['message'] = "Reserved Loop and associated images deleted successfully!";
        } else {
            $returned_data['status'] = 'success';
    ...
    // Summary Reserved Loop ---
    public function summaryTransReservedLoop($auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        $summary = $summary->where('loop_type','reserved_loop')->selectRaw(
    ...
    // Get Reserved Loop Lat-Long
    public function getTransReservedLoopLatLong(Request $request, $auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        $query->where('module_type','reserved_loop');
    ...
    // Bulk Upload ---
    public function bulkUploadTransReservedLoop(Request $request) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        ...
        try {
            Excel::import(new TransReservedLoopImport, $request->file('file'));
