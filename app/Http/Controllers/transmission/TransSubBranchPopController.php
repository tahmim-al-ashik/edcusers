...
use App\Imports\TransSubBranchPopImport;
...
class TransSubBranchPopController extends Controller
{
    // Transmission Branch pop list ---
    public function transSubBranchPopList(Request $request, $auth_id) : JsonResponse
    {
        ...
        if($base_role !== 'admin'){
            $query = $query->where('trans_pops.zone_id', $zone_id);
        }
        $query = $query->where('pop_type', 'sub_branch')
            ...
            ->get([
                ...
                'trans_pops.pop_code',
                'trans_pops.pop_type',
                DB::raw('(SELECT pop_code FROM trans_pops AS tp WHERE tp.id = trans_pops.parent_pop_id) as parent_pop_id'),
    ...
    public function transSubBranchPopDetails($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $transPop = TransPop::query();
        if($base_role !== 'admin'){
            $transPop = $transPop->where('zone_id', $zone_id);
        }
        $transPop = $transPop->leftJoin('trans_companies', 'trans_pops.company_id', '=', 'trans_companies.id')
                            ->with([
                                'subBranchDeviceInfos',
                                'subBranchOutputDeviceInfos',
                                'subBranchCableDetails',
                                'subBranchCoreJoinInfo',
                                'subBranchWorkerInfos'
                            ])
                            ->select(
                                'trans_pops.*',
        ...
        if ($transPop) {
            $details = [
                ...
                'nttn_pop_id' => TransPop::where('id', $transPop->nttn_pop_id)->value('pop_code'),
                'backup_nttn_pop_id' => TransPop::where('id', $transPop->backup_nttn_pop_id)->value('pop_code'),
                'parent_pop_id' => TransPop::where('id', $transPop->parent_pop_id)->value('pop_code'),
                ...
                'out_cable_details' => $transPop->subBranchCableDetails->map(function ($cableDetail) {
                ...
                'in_cable_details' => $transPop->subBranchCableDetails->map(function ($cableDetail) {
                ...
                'pop_input_device_info' => $transPop->subBranchDeviceInfos->map(function ($deviceInfo) {
                    return [
                        'id' => $deviceInfo->id,
                ...
                'pop_rak_info' => $transPop->subBranchDeviceInfos->map(function ($deviceInfo) {
                    return [
                        'id' => $deviceInfo->id,
                ...
                'pop_mk_device_info' => $transPop->subBranchDeviceInfos->map(function ($deviceInfo) {
                    return [
                        'id' => $deviceInfo->id,
                ...
                'pop_output_device_info' => $transPop->subBranchOutputDeviceInfos->map(function ($deviceInfo) {
                    return [
                        'id' => $deviceInfo->id,
                ...
                'images' => $transPop->subBranchImages->map(function ($image) {
                    return [
                        'id' => $image->id,
                ...
                'worker_info' => $transPop->subBranchWorkerInfos->map(function ($workerInfo) {
                    return [
                        'id' => $workerInfo->id,
    ...
    public function createTransSubBranchPop(Request $request, $auth_id) : JsonResponse
    {
            $returned_data = ResponseWrapper::Start();
            ...
            $parentPopCode = TransPop::where('pop_code',$request->get('parent_pop_id'))->where('pop_type','branch')->exists();
            if(!$parentPopCode){
                $returned_data['status'] = 'error';
            ...
            $pop->pop_type = 'sub_branch';
            $pop->pop_main_type = $request->get('pop_type');
            $pop->nttn_pop_id  = TransPop::where('id', $parentPopId)->value('nttn_pop_id');
            $pop->backup_nttn_pop_id   = TransPop::where('id', $parentPopId)->value('backup_nttn_pop_id');
            $pop->parent_pop_id   = $parentPopId;
            ...
            $address->module_type = 'sub_branch';
            ...
            foreach ($inCableData as $inCable) {
                ...
                $inCableModel->module_type = 'sub_branch';
            ...
            foreach ($outCableData as $outCable) {
                ...
                $outCableModel->module_type = 'sub_branch';
            ...
            $popDeviceInfo->zone_id = $zone_id;
            $popDeviceInfo->trans_id = $popId;
            $popDeviceInfo->module_type = 'sub_branch';
            ...
            foreach ($popOutputDeviceInfos as $popOutputDeviceInfo) {
                ...
                $popOutputModel->zone_id = $zone_id;
                $popOutputModel->trans_id = $popId;
                $popOutputModel->module_type = 'sub_branch';
            ...
            $workerInfo->module_type = 'sub_branch';
    ...
    public function editTransSubBranchPop(Request $request, $auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $validated = $request->validate([
            'pop_id' => 'required',
        ...
        if($pop){
            $pop->update([
                ...
                'nttn_pop_id' => TransPop::where('id', $parentPopId)->value('nttn_pop_id'),
                'backup_nttn_pop_id' => TransPop::where('id', $parentPopId)->value('backup_nttn_pop_id'),
                'parent_pop_id' => $parentPopId,
        ...
        foreach ($popOutputDeviceInfos as $popOutputItem) {
            ...
            if($deviceId){
                $popOutputDeviceInfo = TransPopOutputDeviceInfo::where('trans_id', $id)->where('id', $popOutputItem['id'])->first();
    ...
    public function deleteTransSubBranchPop($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $latLongDeleted = $latLongDeleted->where('trans_id', $id)->where('module_type','sub_branch')->delete();

        $outputDeviceDeleted = TransPopOutputDeviceInfo::query();
        if($base_role !== 'admin'){
            $outputDeviceDeleted = $outputDeviceDeleted->where('zone_id', $zone_id);
        }
        $outputDeviceDeleted = $outputDeviceDeleted->where('trans_id', $id)->where('module_type','sub_branch')->delete();

        $deviceDeleted = TransPopDeviceInfo::query();
        if($base_role !== 'admin'){
            $deviceDeleted = $deviceDeleted->where('zone_id', $zone_id);
        }
        $deviceDeleted = $deviceDeleted->where('trans_id', $id)->where('module_type','sub_branch')->delete();

        $cableDeleted = TransCableDetail::query();
        if($base_role !== 'admin'){
            $cableDeleted = $cableDeleted->where('zone_id', $zone_id);
        }
        $cableDeleted = $cableDeleted->where('trans_id', $id)->where('module_type','sub_branch')->delete();

        $workerDeleted = TransWorkerInfo::query();
        if($base_role !== 'admin'){
            $workerDeleted = $workerDeleted->where('zone_id', $zone_id);
        }
        $workerDeleted = $workerDeleted->where('trans_id', $id)->where('module_type','sub_branch')->delete();

        // Fetch all images related to the pop
        $images = TransImage::where('trans_id', $id)->where('module_type','sub_branch')->get();
        ...
        $imageDeleted = $imageDeleted->where('trans_id', $id)->where('module_type','sub_branch')->delete();

        $popDeleted = TransPop::query();
        if($base_role !== 'admin'){
            $popDeleted = $popDeleted->where('zone_id', $zone_id);
        }
        $popDeleted = $popDeleted->where('id', $id)->where('pop_type','sub_branch')->delete();
    ...
    public function summaryTransSubBranchPop($auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        // summary
        $summary = TransPop::query();
        if($base_role !== 'admin'){
            $summary = $summary->where('zone_id', $zone_id);
        }
        $summary = $summary->where('pop_type','sub_branch')->selectRaw(
    ...
    // Lat Long list ---
    public function getTransSubBranchPopLatLong(Request $request, $auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        $query->where('module_type', 'sub_branch');
    ...
    // Bulk Upload ---
    public function bulkUploadTransSubBranchPop(Request $request) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        ...
        try {
            Excel::import(new TransSubBranchPopImport, $request->file('file'));
