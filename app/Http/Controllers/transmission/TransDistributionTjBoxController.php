...
class TransDistributionTjBoxController extends Controller
{
    ...
    public function transDistributionTjBoxList(Request $request, $auth_id) : JsonResponse
    {
    ...
    public function transDistributionTjBoxDetails($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $transDistributionTjBox = TransTjBox::query();
        if($base_role !== 'admin'){
            $transDistributionTjBox = $transDistributionTjBox->where('trans_tj_boxes.zone_id', $zone_id);
        }
        $transDistributionTjBox = $transDistributionTjBox->leftJoin('trans_pops', 'trans_tj_boxes.pop_id', '=', 'trans_pops.id')
                            ->with([
    ...
    public function createTransDistributionTjBox(Request $request, $auth_id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        ...
        foreach ($coreJoinInfos as $coreJoinInfo) {
            ...
            $coreJoin->in_fiber_id = TransCableDetail::where('fiber_code',$coreJoinInfo['in_fiber_id'])->where('trans_id',$tjId)->where('module_type','distribution_tj')->where('cable_type','in_cable')->value('id') ?? null;
            $coreJoin->out_fiber_id = TransCableDetail::where('fiber_code',$coreJoinInfo['out_fiber_id'])->where('trans_id',$tjId)->where('module_type','distribution_tj')->where('cable_type','out_cable')->value('id') ?? null;
    ...
    public function editTransDistributionTjBox(Request $request, $auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $validated = $request->validate([
            'tj_box_id' => 'required',
        ...
        $TjBox = TransTjBox::where('id', $id)->where('tj_box_type','distribution_tj')->first();
        if($TjBox){
            $TjBox->update([
                // 'zone_id' => $zone_id,
                // 'pop_id' => $parentPopId,
        ...
        // TransLatLong::where('trans_id', $id)->where('module_type','distribution_tj')->update([
        //     'zone_id' => $zone_id,
        //     'trans_id' => $id,
        ...
        $address = TransLatLong::where('trans_id', $id)->where('module_type', 'distribution_tj')->first();
        if($address){
            $address->update([
                'latitude' => $request->get('latitude'),
                'longitude' => $request->get('longitude'),
    ...
    public function deleteTransDistributionTjBox($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $latLongDeleted = $latLongDeleted->where('trans_id', $id)->where('module_type','distribution_tj')->delete();

        $joinInfoDeleted = TransCoreJoinInfo::query();
        if($base_role !== 'admin'){
            $joinInfoDeleted = $joinInfoDeleted->where('zone_id', $zone_id);
        }
        $joinInfoDeleted = $joinInfoDeleted->where('trans_id', $id)->where('module_type','distribution_tj')->delete();

        $cableDeleted = TransCableDetail::query();
        if($base_role !== 'admin'){
            $cableDeleted = $cableDeleted->where('zone_id', $zone_id);
        }
        $cableDeleted = $cableDeleted->where('trans_id', $id)->where('module_type','distribution_tj')->delete();

        $workerDeleted = TransWorkerInfo::query();
        if($base_role !== 'admin'){
            $workerDeleted = $workerDeleted->where('zone_id', $zone_id);
        }
        $workerDeleted = $workerDeleted->where('trans_id', $id)->where('module_type','distribution_tj')->delete();

        $splitterDeleted = TransTjBoxSplitters::query();
        if($base_role !== 'admin'){
            $splitterDeleted = $splitterDeleted->where('zone_id', $zone_id);
        }
        $splitterDeleted = $splitterDeleted->where('trans_id', $id)->where('module_type','distribution_tj')->delete();
        ...
        $images = $images->where('trans_id', $id)->where('module_type','distribution_tj')->get();
        ...
        $imageDeleted = $imageDeleted->where('trans_id', $id)->where('module_type','distribution_tj')->delete();

        $distributionTjDeleted = TransTjBox::query();
        if($base_role !== 'admin'){
            $distributionTjDeleted = $distributionTjDeleted->where('zone_id', $zone_id);
        }
        $distributionTjDeleted = $distributionTjDeleted->where('id', $id)->where('tj_box_type','distribution_tj')->delete();
    ...
    // Summary Tj Box ---
    public function summaryTransDistributionTjBox($auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        $summary = $summary->where('tj_box_type','distribution_tj')->selectRaw(
    ...
    public function getLatestSplitterId() : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        // Id
        $query = TransTjBoxSplitters::query();
        $query = $query->where('module_type', 'distribution_tj')->latest('id')->first('id');
    ...
    public function getTransDistributionTjBoxLatLong(Request $request, $auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
