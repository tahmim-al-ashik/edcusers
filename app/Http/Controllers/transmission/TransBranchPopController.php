...
class TransBranchPopController extends Controller
{
    ...
    public function transBranchPopList (Request $request, $auth_id) : JsonResponse
    {
        ...
        $query = $query->where('trans_pops.pop_type', 'branch');

        if($base_role !== 'admin'){
            $query = $query->where('trans_pops.zone_id', $zone_id);
        }

        $query = $query->leftJoin('trans_companies', 'trans_pops.company_id', '=', 'trans_companies.id')
    ...
    // Transmission Branch pop list ---
    public function transBranchPops (Request $request, $auth_id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
    ...
    // Transmission Branch pop list ---
    public function transBranchPopsShadhin () : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();

        $query = TransPop::query();
        $query = $query->where('zone_id', 10000000);
        $query = $query->where('pop_type', 'branch')->orderBy('trans_pops.id', 'desc')
            ->get([
                'trans_pops.id',
                'trans_pops.pop_sl_no'
            ]);
    ...
    public function transBranchPopsTree ($auth_id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
    ...
    public function transBranchPopDetails($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        if($base_role !== 'admin'){
            $transPop = $transPop->where('trans_pops.zone_id', $zone_id);
        }
        $transPop = $transPop->where('trans_pops.id', $id)
    ...
    public function createTransBranchPop(Request $request, $auth_id) : JsonResponse
    {
            $returned_data = ResponseWrapper::Start();
            ...
            $pop->pop_sl_no = $request->get('pop_sl_no');
            $pop->pop_type = 'branch';
    ...
    public function editTransBranchPop(Request $request, $auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $validated = $request->validate([
            'pop_id' => 'required',
        ...
        if($pop){
            $pop->update([
                ...
                'pop_sl_no' => $request->get('pop_sl_no'),
                'pop_type' => 'branch',
    ...
    public function deleteTransBranchPop($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();

        // Validate zone and get base role
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        }elseif($base_role === 'sub_agent') {
        ...
        if ($base_role !== 'admin') {
            $baseCondition['zone_id'] = $zone_id;
        }

        // Delete related records
        $latLongDeleted = TransLatLong::where($baseCondition)->delete();
        $outputDeviceDeleted = TransPopOutputDeviceInfo::where($baseCondition)->delete();
        $deviceDeleted = TransPopDeviceInfo::where($baseCondition)->delete();
        $cableDeleted = TransCableDetail::where($baseCondition)->delete();
        $workerDeleted = TransWorkerInfo::where($baseCondition)->delete();

        // Handle images
        $imageQuery = TransImage::where($baseCondition);
        ...
        $imageDeleted = $imageQuery->delete();

        // Delete main pop record
        $popCondition = [
            'id' => $id,
            'pop_type' => 'branch'
        ];
        if ($base_role !== 'admin') {
            $popCondition['zone_id'] = $zone_id;
        }
        $popDeleted = TransPop::where($popCondition)->delete();

        $returned_data['status'] = ($latLongDeleted && $outputDeviceDeleted && $deviceDeleted &&
                                  $cableDeleted && $workerDeleted && $popDeleted) ? 'success' : 'error';
        $returned_data['message'] = $returned_data['status'] === 'success' ?
            "Pop and associated images deleted successfully!" :
    ...
    public function summaryTransBranchPop($auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        ...
        $summary = $summary->where('pop_type','branch')->selectRaw(
    ...
    public function getTransBranchPopLatLong(Request $request, $auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        ...
        $query->where('trans_lat_longs.module_type','branch');
    ...
    public function getTransLatLong(Request $request, $auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        ...
        $query->leftJoin('trans_pops','trans_pops.id', '=', 'trans_lat_longs.trans_id')->where('module_type','branch');
        $query->leftJoin('trans_pops','trans_pops.id', '=', 'trans_lat_longs.trans_id')->where('module_type','nttn');
