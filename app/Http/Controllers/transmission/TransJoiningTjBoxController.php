...
use App\Imports\TransJoiningTjBoxImport;
...
class TransJoiningTjBoxController extends Controller
{
    // Joining Tj Box list ---
    public function transJoiningTjBoxList(Request $request, $auth_id) : JsonResponse
    {
        ...
        $query = $query->where('tj_box_type','joining_tj')->leftJoin('trans_pops', 'trans_tj_boxes.pop_id', '=', 'trans_pops.id')
    ...
    // Joining Tj Box Single Details ---
    public function transJoiningTjBoxDetails($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $transJoiningTjBox = TransTjBox::query();
        if($base_role !== 'admin'){
            $transJoiningTjBox = $transJoiningTjBox->where('trans_tj_boxes.zone_id', $zone_id);
        }
        $transJoiningTjBox = $transJoiningTjBox->where('trans_tj_boxes.id', $id)
                            ->leftJoin('trans_pops', 'trans_tj_boxes.pop_id', '=', 'trans_pops.id')
                            ->with(['joiningCableDetails','joiningCoreJoinInfo','joiningWorkerInfos','joiningImages','joiningLatLong'])
                            ->select(
                                'trans_tj_boxes.*',
        ...
        if ($transJoiningTjBox) {
            $details = [
                'id' => $transJoiningTjBox->id,
                'pop_id' => $transJoiningTjBox->pop_id,
                'pop_code' => TransPop::where('id',$transJoiningTjBox->pop_id)->value('pop_code'),
                'tj_box_code' => $transJoiningTjBox->tj_box_code,
                'olt_port' => $transJoiningTjBox->olt_port,
                'tj_box_type' => $transJoiningTjBox->tj_box_type,
                'parent_tj_box_code' => TransTjBox::where('id',$transJoiningTjBox->parent_tj_box_id)->value('tj_box_code'),
                'latitude' => $transJoiningTjBox->latitude,
                'longitude' => $transJoiningTjBox->longitude,
                'address_direction' => $transJoiningTjBox->address_direction,
                'added_by_uid' => $transJoiningTjBox->added_by_uid,
                'updated_by_uid' => $transJoiningTjBox->updated_by_uid,
                'comments' => $transJoiningTjBox->comments,
                'status' => $transJoiningTjBox->status,
                'created_at' => $transJoiningTjBox->created_at,
                'updated_at' => $transJoiningTjBox->updated_at,

                'out_cable_details' => $transJoiningTjBox->joiningCableDetails->map(function ($cableDetail) {
                ...
                'in_cable_details' => $transJoiningTjBox->joiningCableDetails->map(function ($cableDetail) {
                ...
                'core_join_infos' => $transJoiningTjBox->joiningCoreJoinInfo->map(function ($joinInfos) {
                    return [
                        'id' => $joinInfos->id,
                ...
                'images' => $transJoiningTjBox->joiningImages->map(function ($image) {
                    return [
                        'id' => $image->id,
                ...
                'worker_info' => $transJoiningTjBox->joiningWorkerInfos->map(function ($workerInfo) {
                    return [
                        'id' => $workerInfo->id,
    ...
    // Create Joining Tj Box ---
    public function createTransJoiningTjBox(Request $request, $auth_id) : JsonResponse
    {
            $returned_data = ResponseWrapper::Start();
            ...
            $address->module_type = 'joining_tj';
            ...
            foreach ($inCableData as $inCable) {
                ...
                $inCableModel->module_type = 'joining_tj';
            ...
            foreach ($outCableData as $outCable) {
                ...
                $outCableModel->module_type = 'joining_tj';
            ...
            foreach ($coreJoinInfos as $coreJoinInfo) {
                ...
                $coreJoin->module_type = 'joining_tj';
            ...
            $workerInfo->module_type = 'joining_tj';
            ...
            $returned_data['status'] = 'success';
            $returned_data['message'] = 'Joining tj box added successfully!';
            $returned_data['results'] = $tjId;
            return ResponseWrapper::End($returned_data);
    }

    // Edit Joining Tj Box ---
    public function editTransJoiningTjBox(Request $request, $auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $validated = $request->validate([
            'tj_box_id' => 'required',
        ...
        $returned_data['status'] = 'success';
        $returned_data['message'] = 'Joining Tj Box data updated successfully!';
        return ResponseWrapper::End($returned_data);
    }

    // Delete Joining Tj Box ---
    public function deleteTransJoiningTjBox($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $latLongDeleted = $latLongDeleted->where('trans_id', $id)->where('module_type','joining_tj')->delete();

        $joinInfoDeleted = TransCoreJoinInfo::query();
        if($base_role !== 'admin'){
            $joinInfoDeleted = $joinInfoDeleted->where('zone_id', $zone_id);
        }
        $joinInfoDeleted = $joinInfoDeleted->where('trans_id', $id)->where('module_type','joining_tj')->delete();

        $cableDeleted = TransCableDetail::query();
        if($base_role !== 'admin'){
            $cableDeleted = $cableDeleted->where('zone_id', $zone_id);
        }
        $cableDeleted = $cableDeleted->where('trans_id', $id)->where('module_type','joining_tj')->delete();

        $workerDeleted = TransWorkerInfo::query();
        if($base_role !== 'admin'){
            $workerDeleted = $workerDeleted->where('zone_id', $zone_id);
        }
        $workerDeleted = $workerDeleted->where('trans_id', $id)->where('module_type','joining_tj')->delete();
        ...
        $images = $images->where('trans_id', $id)->where('module_type','joining_tj')->get();
        ...
        $imageDeleted = $imageDeleted->where('trans_id', $id)->where('module_type','joining_tj')->delete();

        $joiningTjDeleted = TransTjBox::query();
        if($base_role !== 'admin'){
            $joiningTjDeleted = $joiningTjDeleted->where('zone_id', $zone_id);
        }
        $joiningTjDeleted = $joiningTjDeleted->where('id', $id)->where('tj_box_type','joining_tj')->delete();
        if ($latLongDeleted && $joinInfoDeleted && $cableDeleted && $workerDeleted && $joiningTjDeleted) {
            $returned_data['status']  = 'success';
            $returned_data['message'] = "Joining Tj and associated images deleted successfully!";
        } else {
            $returned_data['status'] = 'success';
    ...
    // Summary Tj Box ---
    public function summaryTransJoiningTjBox($auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        if($base_role !== 'admin'){
            $summary = $summary->where('trans_tj_boxes.zone_id', $zone_id);
        }
        $summary = $summary->where('tj_box_type', 'joining_tj')->selectRaw(
    ...
    // Get Joining Tj Box Lat-Long
    public function getTransJoiningTjBoxLatLong(Request $request, $auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        $query->where('module_type', 'joining_tj');
    ...
    // Bulk Upload Hello---
    public function bulkUploadTransJoiningTjBox(Request $request) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        ...
        try {
            Excel::import(new TransJoiningTjBoxImport, $request->file('file'));
