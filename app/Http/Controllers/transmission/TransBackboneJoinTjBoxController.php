...
class TransBackboneJoinTjBoxController extends Controller
{
    ...
    public function transBackboneJoinTjBoxList(Request $request, $auth_id) : JsonResponse
    {
    ...
    public function transBackboneJoinTjBoxDetails($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
    ...
    public function createTransBackboneJoinTjBox(Request $request, $auth_id) : JsonResponse
    {
            $returned_data = ResponseWrapper::Start();
            $base_role = User::where('id', $auth_id)->value('base_role');
            if($base_role === 'agent') {
    ...
    public function editTransBackboneJoinTjBox(Request $request, $auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $validated = $request->validate([
            'tj_box_id' => 'required',
    ...
    public function deleteTransBackboneJoinTjBox($auth_id, $id) : JsonResponse
    {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
        ...
        $latLongDeleted = $latLongDeleted->where('trans_id', $id)->where('module_type','backbone_join_tj')->delete();

        $joinInfoDeleted = TransCoreJoinInfo::query();
        if($base_role !== 'admin'){
            $joinInfoDeleted = $joinInfoDeleted->where('zone_id', $zone_id);
        }
        $joinInfoDeleted = $joinInfoDeleted->where('trans_id', $id)->where('module_type','backbone_join_tj')->delete();

        $cableDeleted = TransCableDetail::query();
        if($base_role !== 'admin'){
            $cableDeleted = $cableDeleted->where('zone_id', $zone_id);
        }
        $cableDeleted = $cableDeleted->where('trans_id', $id)->where('module_type','backbone_join_tj')->delete();

        $workerDeleted = TransWorkerInfo::query();
        if($base_role !== 'admin'){
            $workerDeleted = $workerDeleted->where('zone_id', $zone_id);
        }
        $workerDeleted = $workerDeleted->where('trans_id', $id)->where('module_type','backbone_join_tj')->delete();
        ...
        $imageDeleted = $imageDeleted->where('trans_id', $id)->where('module_type','backbone_join_tj')->delete();

        $backboneJoinTjDeleted = TransTjBox::query();
        if($base_role !== 'admin'){
            $backboneJoinTjDeleted = $backboneJoinTjDeleted->where('zone_id', $zone_id);
        }
        $backboneJoinTjDeleted = $backboneJoinTjDeleted->where('id', $id)->where('tj_box_type','backbone_join_tj')->delete();
    ...
    // Summary Tj Box ---
    public function summaryTransBackboneJoinTjBox($auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
        ...
        $summary = $summary->where('tj_box_type','backbone_join_tj')->selectRaw(
    ...
    public function getTransBackboneJoinTjBoxLatLong(Request $request, $auth_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        $base_role = User::where('id', $auth_id)->value('base_role');
        if($base_role === 'agent') {
            $zone_id = CorporateAgent::where('uid', $auth_id)->value('client_id');
