...
class GraphSalesPointUserController extends Controller
{
    /**
     * Sales Point User Count Bar Chart.
     */
    public function barchart() : JsonResponse {
        ...
        $totalByMonthAndStatus = SalesPoint::selectRaw('YEAR(created_at) AS year, MONTH(created_at) AS month, status, COUNT(*) AS total')
        ...
        $returned_data['results']['total'] = SalesPoint::count();

        return ResponseWrapper::End($returned_data);
    }

    /**
     * Sales Point line chart
     */
    public function salesPointLineChart($days, $type) : JsonResponse {
        $returned_data = ResponseWrapper::Start();

        // Calculate the date x days ago
        $startDate = now()->subDays($days)->startOfDay();

        $query = SalesPoint::selectRaw(
