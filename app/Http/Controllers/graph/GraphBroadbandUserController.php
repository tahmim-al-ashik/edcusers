...
class GraphBroadbandUserController extends Controller
{
    /**
     * Zone & month Wise Bar Chart For Total Wifi User
     */
    public function broadbandUserBarChart($user_id) : JsonResponse {
        ...
        $userCounts = DB::table('payments')
            ...
            ->where('iu.package_type', 'broadband')
    ...
    /**
     * Zone & month Wise Bar Chart For Total Payment of Broadband User
     */
    public function broadbandTotalPaymentBarChart($user_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        ...
        if ($client) {
            $userCountsQuery->where('internet_users.zone_id', $user_id)->where('package_type','broadband');
        } elseif ($agent) {
            $userCountsQuery->where('internet_users.agent_id', $user_id)->where('package_type','broadband');
        } elseif ($sub_agent) {
            $userCountsQuery->where('internet_users.sub_agent_id', $user_id)->where('package_type','broadband');
        }
    ...
    /**
     * Zone Wise Total Broadband User
     */
    public function broadbandTotalUser($user_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();

        set_time_limit(0);

        $userCounts = InternetUsers::join('users', 'users.id', 'internet_users.uid')
        ->selectRaw('COUNT(internet_users.uid) AS total')
        ->where('users.base_role','user')
        ->where(function($query) use ($user_id) {
            $query->where(function($query) use ($user_id) {
                $client = CorporateClient::where('uid', $user_id)->exists();
                if ($client) {
                    $query->where('internet_users.zone_id', $user_id)->where('package_type','broadband');
                } else {
                    $agent = CorporateAgent::where('uid', $user_id)->exists();
                    if ($agent) {
                        $query->where('internet_users.agent_id', $user_id)->where('package_type','broadband');
                    } else {
                        $sub_agent = CorporateSubAgent::where('uid', $user_id)->exists();
                        if ($sub_agent) {
                            $query->where('internet_users.sub_agent_id', $user_id)->where('package_type','broadband');
                        }
                    }
                }
            });
    ...
    /**
     * Zone Wise Total Broadband Payment
     */
    public function broadbandTotalPayment($user_id) : JsonResponse {
        $returned_data = ResponseWrapper::Start();
        ...
        if ($client) {
            $userCounts->where('internet_users.zone_id', $user_id)->where('package_type','broadband');
        } elseif ($agent) {
            $userCounts->where('internet_users.agent_id', $user_id)->where('package_type','broadband');
        } elseif($sub_agent) {
            $userCounts->where('internet_users.sub_agent_id', $user_id)->where('package_type','broadband');
        }
