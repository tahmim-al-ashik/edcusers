...
class TransSubBranchPopImport implements ToModel, WithHeadingRow
{
    ...
    private function checkIds(array $row)
    {
        if (!TransPop::where('pop_code', $row['parent_pop_id'])->exists()) {
            throw new \Exception(json_encode('No parent pop found!'));
        }
    }
    ...
    private function createTransPop(array $row)
    {
        ...
        //Log::info($parentPopId);
        $pop = new TransPop();
        $pop->company_id = TransPop::where('id', $parentPopId)->value('company_id');
